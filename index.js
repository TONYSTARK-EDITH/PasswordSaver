...
const setPasswords = length => {
    ...
    const generateRandomCapitalization = string => string.toUpperCase().split(' ').map(char => {
        return char.slice(0, 16).toLowerCase() + char.slice(17).toUpperCase() + splChars.charAt(Math.floor(Math.random() * splChars.length))
    }).join(' ');

    class Sha512 {
        constructor(msg) {
            this._hash = Sha512.hash(msg).toString()
        }
